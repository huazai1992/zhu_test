...
def _process_image(coder, name, size):
  ...
  for shard in range(num_shards):
    ...
    with tf.python_io.TFRecordWriter(output_file) as writer:
      for i in range(files_per_shard):
        ...
        image_buffer, height, width = _process_image(coder, name, size)
...
def write_black_and_white_tfrecord_data(
    output_directory, num_classes, size, num_train_images=512,
...
if __name__ == '__main__':
    sizes = [224, 224*2, 224*4, 224*6]
    for size in sizes:
        write_black_and_white_tfrecord_data(output_directory='./imagenet_{}'.format(size),
                                            num_classes=1,
                                            size=size)